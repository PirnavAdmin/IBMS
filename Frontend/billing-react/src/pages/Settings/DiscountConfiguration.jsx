...
import { ArrowBackOutlined, LocalOfferOutlined, SaveOutlined, RestartAlt } from '@mui/icons-material';
...
export function DiscountConfiguration() {
  ...
  const disabled = loading || saving || savingRoles;
  const load = async () => {
    setLoading(true); setError('');
    try {
      const [nextConfig, nextRoles] = await Promise.all([phase5Api.getDiscountConfiguration(), phase5Api.getDiscountRolePermissions()]);
      setConfig(nextConfig); setSaved(nextConfig); setRoles(nextRoles); setSavedRoles(nextRoles);
  ...
  const save = async () => {
    ...
    catch (failure) { setError(failure.message); if (failure.cause?.response?.status === 409) await load(); }
  ...
  return <main className="settings-page"><header className="settings-page-header"><div><p className="settings-eyebrow">Settings / Administration</p><h1>Discount Configuration</h1><p>Control discount limits and who can apply or override them.</p></div><div className="settings-actions"><Button variant="text" startIcon={<ArrowBackOutlined />} onClick={() => navigate('/settings')}>Back to Settings</Button><Button variant="outlined" startIcon={<RestartAlt />} disabled={disabled || !dirty} onClick={() => setConfig(saved)}>Reset</Button><Button variant="contained" startIcon={<SaveOutlined />} disabled={disabled || !dirty || Object.keys(configErrors).length > 0} onClick={save}>{saving ? 'Saving...' : 'Save Changes'}</Button></div></header>
    {error && <Alert severity="error" sx={{ mb: 2 }} action={<Button disabled={disabled} onClick={load}>Retry</Button>}>{error}</Alert>}
