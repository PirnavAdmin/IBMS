...
export function ChargesConfiguration() {
  ...
  useEffect(() => { load(); }, []);
  const rememberCharge = (result) => setCharges(current => current.some(item => item.id === result.id) ? current.map(item => item.id === result.id ? result : item) : [...current, result]);
  ...
  const submit = async () => {
    ...
    try { const result = dialog === 'new' ? await phase5Api.createCharge(form) : await phase5Api.updateCharge(form); rememberCharge(result); setDialog(null); setToast(dialog === 'new' ? `${result.name} added.` : `${result.name} updated.`); await load(); }
  ...
  const changeStatus = async () => {
    ...
    try { const result = await phase5Api.updateCharge({ ...confirm, status: next }); rememberCharge(result); setConfirm(null); setToast(`${result.name} ${next === 'Active' ? 'activated' : 'deactivated'}.`); await load(); }
    catch (error) { setApiError(error.message); }
    finally { requestLock.current = false; setBusy(false); }
  };
  return <main className="settings-page"><SettingsPageHeader title="Charges Configuration" description="Configure charges for invoices."><Button variant="contained" startIcon={<AddOutlined />} disabled={busy || loading} onClick={openNew}>Add Charge</Button></SettingsPageHeader>
    {apiError && charges.length > 0 && !dialog && !confirm && <DashboardErrorState title="Unable to load charges" message={apiError} onRetry={load} />}
    <section className="settings-panel settings-full"><div className="settings-panel-heading"><div><h2>Configured Charges</h2><p>Shipping, handling, convenience, late fee, and custom charges.</p></div></div>{loading ? <div className="settings-empty">Loading charges...</div> : apiError && !charges.length ? <DashboardErrorState title="Unable to load charges" message={apiError} onRetry={load} /> : charges.length ? <div className="settings-table-wrap"><table className="settings-table"><thead><tr><th>Charge Name</th><th>Charge Code</th><th>Charge Type</th><th>Calculation</th><th>Value</th><th>Taxable</th><th>Status</th><th>Actions</th></tr></thead><tbody>{charges.map((charge) => <tr key={charge.id}><td><strong>{charge.name}</strong><small className="settings-cell-note">{charge.description}</small></td><td>{charge.code}</td><td>{charge.type}</td><td>{charge.calculationType}</td><td>{displayValue(charge)}</td><td>{charge.taxable ? 'Yes' : 'No'}</td><td><span className={`settings-status ${charge.status === 'Active' ? 'active' : 'inactive'}`}>{validStatus(charge.status) ? charge.status : `Unknown (${charge.status || 'empty'})`}</span></td><td><div className="settings-row-actions"><Button size="small" startIcon={<EditOutlined />} disabled={busy || loading} onClick={() => openEdit(charge)}>Edit</Button><Button size="small" disabled={busy || loading || !validStatus(charge.status)} color={charge.status === 'Active' ? 'error' : 'primary'} onClick={() => { setApiError(''); setConfirm(charge); }}>{charge.status === 'Active' ? 'Deactivate' : 'Activate'}</Button></div></td></tr>)}</tbody></table></div> : <div className="settings-empty"><h2>No charges configured yet.</h2><p>Add a charge to begin defining your billing rules.</p><Button variant="contained" startIcon={<AddOutlined />} disabled={busy || loading} onClick={openNew}>Add Charge</Button></div>}</section>
    <Dialog open={Boolean(dialog)} onClose={() => { if (!busy) setDialog(null); }} fullWidth maxWidth="sm"><DialogTitle>{dialog === 'new' ? 'Add Charge' : 'Edit Charge'}</DialogTitle><DialogContent dividers><fieldset disabled={busy} style={{ border: 0, padding: 0, margin: 0 }}><div className="settings-dialog-form"><TextField autoFocus required label="Charge Name" value={form.name} onChange={(e) => update('name', e.target.value)} error={Boolean(errors.name)} helperText={errors.name} /><TextField required label="Charge Code" value={form.code} onChange={(e) => update('code', e.target.value)} error={Boolean(errors.code)} helperText={errors.code} inputProps={{ maxLength: 64 }} /><div className="settings-select-field"><label htmlFor="charge-type">Charge Type *</label><Select id="charge-type" inputProps={{ "aria-label": "Charge Type" }} value={form.type} onChange={(e) => update('type', e.target.value)} error={Boolean(errors.type)}>{chargeTypes.map((type) => <MenuItem key={type} value={type}>{type}</MenuItem>)}</Select></div><div className="settings-select-field"><label htmlFor="calculation-type">Calculation Type *</label><Select id="calculation-type" inputProps={{ "aria-label": "Calculation Type" }} value={form.calculationType} onChange={(e) => update('calculationType', e.target.value)}>{['Fixed', 'Percentage'].map((type) => <MenuItem key={type} value={type}>{type}</MenuItem>)}</Select></div><TextField required label={`Value${form.calculationType === 'Percentage' ? ' (%)' : ' (₹)'}`} type="number" value={form.value} onChange={(e) => update('value', e.target.value)} error={Boolean(errors.value)} helperText={errors.value} inputProps={{ min: 0, max: form.calculationType === 'Percentage' ? 100 : undefined, step: 'any' }} /><div className="settings-toggle-field"><FormControlLabel control={<Switch checked={form.taxable} onChange={(e) => update('taxable', e.target.checked)} />} label="Taxable" /><div className="settings-select-field"><label htmlFor="charge-status">Status *</label><Select id="charge-status" inputProps={{ "aria-label": "Status" }} value={form.status} onChange={(e) => update('status', e.target.value)} error={Boolean(errors.status)}>{!validStatus(form.status) && <MenuItem disabled value={form.status}>Unknown ({form.status || 'empty'})</MenuItem>}<MenuItem value="Active">Active</MenuItem><MenuItem value="Inactive">Inactive</MenuItem></Select>{errors.status && <span className="settings-field-error">{errors.status}</span>}</div></div>{!validStatus(form.status) && <Alert severity="warning">This record has an unsupported backend status. Select Active or Inactive before saving.</Alert>}<TextField className="settings-form-span" label="Description" multiline minRows={3} value={form.description} onChange={(e) => update('description', e.target.value)} error={Boolean(errors.description)} helperText={errors.description} /></div></fieldset>{apiError && <Alert severity="error" sx={{ mt: 2 }}>{apiError}</Alert>}</DialogContent><DialogActions><Button disabled={busy} onClick={() => setDialog(null)}>Cancel</Button><Button variant="contained" disabled={busy} onClick={submit}>{busy ? 'Saving...' : dialog === 'new' ? 'Add Charge' : 'Save Changes'}</Button></DialogActions></Dialog>
