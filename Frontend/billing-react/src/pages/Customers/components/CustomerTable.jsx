...
export function CustomerTable({ title, rows, columns, selects, dates = true, defaultSort = 'date', defaultDirection = 'desc', filters: controlled, onFiltersChange, children, extra, searchKeys, prioritizeCustomerNames = false, emptyMessage, server }) {
  ...
  const search = useSearchDebounce(filters.search || '');
  useEffect(() => { setPage(0); }, [search]);
  const updateFilters = (next) => { headerSearch?.onSearch?.(next.search || ''); (onFiltersChange || setLocal)(next); setPage(0); };
  const filtered = server ? rows : filterCustomerRows(rows, { ...filters, search }, 'date', searchKeys);
  const ordered = server ? rows : sortCustomerRows(filtered, sort.key, sort.direction);
  const sorted = server ? rows : prioritizeCustomerNames ? rankCustomerNameMatches(ordered, search) : prioritizePrefix(ordered, search, row => searchKeys ? searchKeys.map(key => row[key]) : Object.values(row));
