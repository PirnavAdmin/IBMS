...
import { getCustomerById, getCustomerDetails, getCustomerAudit, getCustomers, updateCustomer } from '../services/customerService';
...
export function useCustomerMutation(id, operation = updateCustomer) {
  ...
  return useMutation({ mutationFn: (args = []) => operation(id, ...args), onSuccess: async () => {
    await Promise.all([['customers'], ['customer', String(id)], ['customer-details', String(id)], ['customer-audit', String(id)]].map(queryKey => client.invalidateQueries({ queryKey })));
  } });
}
