...
function TransactionTable({ kind, rows }) {
  ...
  const columns = invoice ? [{ key: 'invoiceNumber', label: 'Invoice Number' }, date('date', 'Invoice Date'), date('dueDate', 'Due Date'), money('amount', 'Amount'), money('paid', 'Paid Amount'), money('balance', 'Balance'), statusColumn] : [{ key: 'paymentNumber', label: 'Payment ID' }, date('date', 'Payment Date'), { key: 'method', label: 'Payment Method' }, { key: 'reference', label: 'Reference Number' }, { key: 'invoice', label: 'Invoice Number' }, money('amount', 'Amount'), statusColumn];
